...
class GameViewController: UIViewController {
    ...
    var jumbDownwardAction: SCNAction?
    ...
    func setupScene(){
        ...
        // delegate for SCNSceneRendererDelegate
    ...
    func setupPlayer(){
       guard let playerScene = SCNScene(named: "art.scnassets/Chicken.scn") else {
        ...
        if let player = playerScene.rootNode.childNode(withName: "player", recursively: true){
            ...
            playerNode = player
            playerNode.position = SCNVector3(x:0,y:0.2,z:0)
    ...
    func jumpForward(){
        ...
        if let action = jumpForwardAction{
            playerNode.runAction(action,completionHandler: {
                self.checkBlocks()
                self.score += 1
                self.gameHUD.pointLabel?.text = "\(self.score)"
            })
            addLanes()
        }
    ...
    func removeUnusedLanes(){
        ...
        for child in mapNode.childNodes{
            if !sceneView.isNode(child, insideFrustumOf: cameraNode) && child.worldPosition.z > playerNode.worldPosition.z{
                ...
                lanes.removeFirst()
                print("remove unused lanes")
            }
        }
        
    }
    // update the camera with the player
    ...
    func addActions(for trafficNode:TrafficNode){
        ...
        // cars and trucks will speed up diffrent
...
extension GameViewController: SCNPhysicsContactDelegate{
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        case PhysicsCatagory.chicken | PhysicsCatagory.vehicle:
            gameOver()
        
        case  PhysicsCatagory.vegetation | PhysicsCatagory.collisionTestFront :
            frontBlocked = true
        
        case  PhysicsCatagory.vegetation | PhysicsCatagory.collisionTestRight:
            rightBlocked = true
        
        case PhysicsCatagory.vegetation | PhysicsCatagory.collisionTestLeft:
            leftBlocked = true
        default:
            break
        }
